...
async function ensureEccLocalSettings({ target, dryRun, silent = false }) {
...
export async function setupEcc({ target, dryRun = false, configurePlugin = true, silent = false }) {
  if (configurePlugin) await ensureEccLocalSettings({ target, dryRun, silent });
  let status = hasEccPlugin() ? "installed" : "manual-plugin-install-required";

  if (status === "installed") {
    if (!silent) printSummary("ECC", [["Status", "plugin detected in Claude Code"]]);
  } else if (process.env.REPO_PATTERN_ECC_SETUP_CMD) {
    if (!silent) printSummary("ECC", [["Status", "using REPO_PATTERN_ECC_SETUP_CMD"]]);
    if (dryRun) {
      if (!silent) console.log(`[dry-run] REPO_PATTERN_TARGET=${target} ${process.env.REPO_PATTERN_ECC_SETUP_CMD}`);
      status = "dry-run";
    } else {

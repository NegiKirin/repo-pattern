...
This repository is a minimal ECC-first Claude Code setup and migrator.
...
`setup` must run or attempt ECC setup automatically.

## Safety

* Never commit `.claude/settings.local.json`.
* Never hardcode machine paths in `.mcp.json`.
* Keep `.claude/settings.example.json` with `hooks: {}`; do not commit local `.claude/settings.json`.
...
Canonical files used by `setup` live directly in:
...
This project is indexed by GitNexus as **repo-pattern** (439 symbols, 838 relationships, 37 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
